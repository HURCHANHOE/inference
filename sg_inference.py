...
from sglang.backend import LLMBackend
import os
...
# llms = sgl.Runtime(
#     MODEL_ID,
#     backend=LLMBackend.VLLM,  # vLLM 백엔드 사용
#     max_tokens=512,           # 생성할 최대 토큰 수
#     temperature=0.1,          # 생성 랜덤성 조절
#     tensor_parallel_size=1,    # GPU 병렬 처리 설정 (사용 가능한 GPU 수에 따라 조정)
#     chat_template="llama-3"
# )

llms = sgl.Runtime(
    MODEL_ID,  # GGUF 모델 파일 경로
    backend=LLMBackend.LLAMACPP,  # llama.cpp 백엔드 사용
    temperature=0.1,
    n_ctx=2048,       # 컨텍스트 길이
    n_gpu_layers=-1,  # 가능한 경우 모든 레이어를 GPU에 로드 (-1)
    stop=["\n\n", "\n"],
    f16_kv=True,
    verbose=False,
    chat_template="llama-3",
...
# 내장 chat_completion_to_prompt 기능 사용
@sgl.function
def create_chat_completion(s, messages):
    # sglang의 내장 함수로 메시지 포맷팅
    formatted_prompt = llms.chat_completion_to_prompt(messages)
    
    # 포맷팅된 프롬프트 전달 및 응답 생성
    s += sgl.text(formatted_prompt)
    s += sgl.gen("response")
...
while True:
    ...
    result = create_chat_completion.using(llms).run(
        messages=[
